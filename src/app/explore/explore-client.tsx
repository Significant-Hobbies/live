...
import { ArrowRight, CalendarDays, Layers3, LayoutList, Search, Sparkles } from 'lucide-react';
        ...
        {top3Hobbies.length > 0 && (
          <div className="mt-4 flex flex-wrap gap-1">
            {top3Hobbies.map((hobby) => (
              <span
                key={hobby}
                className="inline-block rounded-full border border-border bg-card/40 px-2.5 py-0.5 text-xs text-muted-foreground"
              >
                {hobby}
              </span>
            ))}
        ...
        {visiblePhases.length > 0 && (
          <div className="mt-4 space-y-2">
            {visiblePhases.map((phase, index) => (
              <div
                key={phase.id}
                className="rounded-lg border border-border bg-card/40/70 px-3 py-2"
              >
                <div className="flex items-center gap-2">
                  <span
                    className={`h-2 w-2 rounded-full ${PHASE_COLORS[index % PHASE_COLORS.length]}`}
                  />
                  <p className="truncate text-xs font-semibold text-foreground">{phase.label}</p>
                </div>
