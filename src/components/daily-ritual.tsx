...
export function DailyRitual({
  ...
  const noveltyRecords =
    initialJournal && !journalEntries.some((entry) => entry.dayDate === today)
  ...
  async function handleSave() {
    ...
    try {
      ...
      window.dispatchEvent(
        new CustomEvent('journal:entry-saved', {
          detail: { dayDate: today, amEntry: amEntry || null, pmEntry: pmEntry || null },
        })
      );
      setSaved(true);
  ...
  return (
    <div className="mx-auto max-w-5xl space-y-6 px-4 py-6 sm:py-8">
      ...
      {/* ─── Ritual header ─── */}
      <section
        className={`relative overflow-hidden rounded-[1.5rem] px-5 py-5 shadow-[0_10px_30px_rgba(66,55,22,0.08)] sm:px-7 sm:py-6 ${
      ...
      >
        <div className="relative flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
          <div>
            ...
            <p className="mt-2 text-sm text-[#403552]">
              Today is the first day of the rest of your life.
            </p>
          </div>
          <div className="flex items-center gap-2 text-xs font-semibold">
            <span className="rounded-full bg-white/65 px-3 py-2">
              AM {amEntry.trim() ? 'written' : 'open'}
            </span>
            <span className="rounded-full bg-white/65 px-3 py-2">
              PM {pmEntry.trim() ? 'written' : 'open'}
            </span>
            {weeksRemaining !== null && (
              <Link
                href="/life-in-weeks"
                className="rounded-full bg-white/65 px-3 py-2 tabular-nums"
              >
      ...
      {/* ─── One lived thing + journal reflection ─── */}
      <div className="grid gap-6 lg:grid-cols-[minmax(0,1.5fr)_minmax(17rem,0.75fr)] lg:items-start">
        <div className="lg:col-start-2 lg:row-start-1">
          <DailyNewThing
            today={today}
            selectedDate={selectedDate}
            seed={noveltySeed}
            records={noveltyRecords}
            persist={actions.setDailyNovelty}
            preview={preview}
          />
        </div>
        ...
        <section
          ...
          className="scroll-mt-24 overflow-hidden rounded-[1.5rem] bg-white shadow-[0_12px_36px_rgba(66,55,22,0.10)] lg:col-start-1 lg:row-start-1"
        >
              ...
              <div className="space-y-6">
                ...
                {journalContextChoices.length > 0 && (
                  <div className="rounded-xl border border-border/60 bg-background/35 px-4 py-3.5">
                    ...
                    >
                      A private thread back to what this day was part of. It does not create proof
                      or progress.
                    </p>
      ...
      {/* ─── Habits — one calm daily checklist ─── */}
      ...
      <div className="grid gap-6 lg:grid-cols-[1.15fr_0.85fr] lg:items-start">
        <JournalArchive records={noveltyRecords} />
        <DailyNewThingHistory records={noveltyRecords} />
      </div>
    </div>
  );
}
