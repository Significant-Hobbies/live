...
function patchDotNextStore() {
  ...
  for (const entry of readdirSync(dotNextPnpm)) {
    ...
    if (!fullNextDir) {
      console.warn(`  Full next not found for: ${entry.slice(0, 50)}`);
      continue;
    }
    ...
    for (const subdir of ['dist/server', 'dist/shared', 'dist/lib', 'dist/build']) {
      ...
      if (existsSync(src))
        cpSync(src, join(sparseNextDir, subdir), { recursive: true, force: true });
    }
    console.log(`  Done`);
  }
}
...
function patchNftFiles() {
  ...
  const nextEntry = sample.files.find(
    (f) => f.includes('node_modules/next') && !f.includes('.pnpm') && !f.endsWith('package.json')
  );
  if (!nextEntry) {
    console.warn('  Could not find next prefix in nft.json');
    return;
  }
  // Strip the trailing path to get just the prefix (e.g. ../../../node_modules/next)
  const nextPkgPrefix = nextEntry.slice(
    0,
    nextEntry.indexOf('node_modules/next') + 'node_modules/next'.length
  );

  const MISSING_FILES = [
    'dist/server/node-environment.js',
    'dist/server/node-environment-baseline.js',
    'dist/server/node-polyfill-crypto.js',
    'dist/server/request-meta.js',
    'dist/server/base-server.js',
    'dist/server/require.js',
    'dist/server/send-payload.js',
    'dist/server/load-components.js',
    'dist/server/web/utils.js',
    'dist/server/base-http/node.js',
    'dist/server/api-utils/index.js',
    'dist/server/api-utils/node/index.js',
    'dist/server/lib/lru-cache.js',
    'dist/server/route-kind.js',
    'dist/server/lib/streaming-metadata.js',
    'dist/server/web/adapter.js',
    'dist/server/lib/fallback.js',
    'dist/server/lib/postponed-request-body.js',
    'dist/shared/lib/utils.js',
    ...
    'dist/lib/is-error.js',
    'dist/lib/picocolors.js',
  ...
  for (const nftPath of nftFiles) {
    try {
      ...
      const thisNextEntry = d.files.find(
        (f) =>
          f.includes('node_modules/next') && !f.includes('.pnpm') && !f.endsWith('package.json')
      );
      const prefix = thisNextEntry
        ? thisNextEntry.slice(
            0,
            thisNextEntry.indexOf('node_modules/next') + 'node_modules/next'.length
          )
        : nextPkgPrefix;
      let added = 0;
      ...
      if (added > 0) {
        writeFileSync(nftPath, JSON.stringify(d));
        patched++;
      }
    } catch {
      /* skip */
  ...
  console.log(
    `  Patched ${patched}/${nftFiles.length} .nft.json files with ${MISSING_FILES.length} missing entries`
  );
}
